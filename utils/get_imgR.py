# 生成带有旋转角度的目标框算法：
	# 步骤一. 随机角度旋转带有目标框(bbox)图像 --- 得到imgR
	# 步骤二. 由于旋转后四个角落会产生空白，需要进行裁剪
	   # 裁剪要求：不能含有空白部分，包含完整的边界框，剪切后图像大于size：224*224
	   # 裁剪算法：
		# 计算边界框的最小外接矩形，
			# 1）外接矩形已经有部分超出imgR，即与空白部分有相交，回到步骤一（最多10次尝试）
			# 2）仍然在imgR中，则开始扩大剪切区域，计算每条边S可以扩张的距离maxD_S(受size和目标框限制)
			   # 即：移动后S与imgR边界的两个交点距离大于224，
			   # 如：可移动距离为含有0，回到步骤一
				# a. 随机选择一条边s向外扩张（移动），移动距离为计算出的最大范围内随机【0，maxD_s】
				# b. 移动后需要更新其它三条边的最大移动距离
					# -- 相邻的两条边的最大扩张距离受到S与imgR产生的两个新交点
					# -- 对边与该边的距离大于224，所以也会受到影响
					# ** 更新可移动距离为含有0，回到步骤a
				# c. 在剩余的三条边里重复步骤ab直到四条边都扩张完毕
				# d. 剪切扩张后的矩形
