...
def getIOU(true_param, pred_param, ts_h, ts_w):
    """
    计算旋转矩形的交并比（IOU）
    :param true_param: 真实框参数 [x, y, scale, rotate]
    :param pred_param: 预测框参数 [x, y, scale, rotate]
    :param ts_h: 图像高度（用于计算实际尺寸）
    :param ts_w: 图像宽度（用于计算实际尺寸）
    :return: IOU值
    """
    if len(pred_param) == 4:
        # 只有一个缩放参数
    ...
    true_width = true_sX * ts_w
    true_height = true_sY * ts_h
    pred_width = pred_sX * ts_w
    pred_height = pred_sY * ts_h
