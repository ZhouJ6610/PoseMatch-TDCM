...
from utils import CoCo_Dataset, deNormalize, get_center
from utils import getIOU, refine_angle_bisection, refine_scale_x, refine_scale_y
...
th, tw = 36, 36
rootPath = '/workspace/zhouji/dataSets/MS-CoCo/'
test_file = 'data/S2/val.csv'
model_file ='dict/model.pth'
result_file = 'results-single.txt'
...
with open(result_file, 'w', encoding='utf-8') as rf:
    ...
    with torch.no_grad():
        for i in range(min(len(dataset), max_samples)):
            ...
            image, template, true_y, true_x, true_sy, true_sx, true_r = dataset[i]
            ...
            match_success = (pred_score[0, 0, pre_Y, pre_X].item() > 0.1 and IOU > 0.1)
            if match_success:
                success_count += 1
                rf.write(f'样本 {i}\n')
                rf.write('状态: 匹配成功\n')
                ...
                rf.write(f'Time(ms): {(inference_time+post_time)*1000:.4f}\n')
