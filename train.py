...
def get_args_parser():
    ...
    parser.add_argument('--train_imgPath',  default='/workspace/zhouji/dataSets/MS-CoCo/')
    parser.add_argument('--val_imgPath',    default='/workspace/zhouji/dataSets/MS-CoCo/')
...
def val(model, val_loader, device, template_shape):
    ...
    with torch.no_grad():
        for i, (images, bboxes, center_y, center_x, scale_y, scale_x, angle) in enumerate(val_loader):
...
def train(model, train_loader, optimizer, device, template_shape):
    ...
    acccuray, anglediff = 0.0, 0.0
    for i, (images, bboxes, center_y, center_x, scale_y, scale_x, angle) in enumerate(train_loader):
        images = images.to(device, non_blocking=True).float()
        bboxes = bboxes.to(device, non_blocking=True).float()
        torch.cuda.empty_cache()
...
def main(args):
    ...
    model = Model(args.snapshot, device).to(device)
    optimizer = optim.Adam(model.parameters(), lr=args.lr)
    
    template_shape = (args.th, args.tw)
    
    train_dataset = CoCo_Dataset_R(args.train_imgPath, args.train_file, template_shape)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True)
    val_dataset = CoCo_Dataset_R(args.val_imgPath, args.val_file, template_shape)
